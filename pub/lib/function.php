...
function get_list_employee($currDep) {
...
function get_vacations($currDep) {

  $client = ws_init();
  if ($client == null) {
    die("Не удалось получить список отпускников");
  }
...
function get_fired_list($currDep, $post) {
  ...
  if (empty($post['rep-period-from'])) {
    die('Неверная дата начала периода отчета: '.$post['rep-period-from'].'<a href="/?mode=fr">Назад</a>');
  }
  $from = strtotime($post['rep-period-from']);
  if (empty($post['rep-period-to'])) {
    die('Неверная дата завершения периода отчета: '.$post['rep-period-to'].'<a href="/?mode=fr">Назад</a>');
  }
  $to = strtotime($post['rep-period-to']);
  if ($to < $from) {
    die("Дата начала периода отчета ".$post['rep-period-from']." больше даты завершения ".$post['rep-period-to'].'<a href="/?mode=fr">Назад</a>');
  }
  $client = ws_init();
  if ($client == null) {
    die("Не удалось получить данные по увольнениям");
  }
